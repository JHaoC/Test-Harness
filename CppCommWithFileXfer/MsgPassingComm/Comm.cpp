...
bool Sender::sendFile(Message msg)
{
  ...
  std::ifstream sendFile(fileSpec, std::ios::in | std::ios::binary/*,_SH_DENYNO*/);
  if (!sendFile.good())
  {
    std::cout << "\n  can't open \"" << fileSpec << "\"";
    return false;
  }
  std::ofstream debug("debug.txt", std::ios::out | std::ios::binary/*,_SH_DENYNO*/);
  if (!debug.good())
  {
    std::cout << "\n  can't open \"" << "debug.txt" << "\"";
    return false;
  }
  while (true)
  {
    ...
    std::streamsize blockSize = sendFile.gcount();
    debug.write(rwBuffer, blockSize);
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
    ...
    std::ofstream saveStream(fileSpec, std::ios::out | std::ios::binary);
    ...
    while (true)
    {
      ...
      //if (msg.contentLength() < BlockSize)
      //  break;
